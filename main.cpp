...
int main(int argc, const char* argv[]) {
  ...
  server.set_open_handler([&server, &connIdToRelay, &nextConnId, &events](auto connHandle) {
    ...
    const auto sendMessage = [&server, connHandle](const std::string& msg) {
      std::cout << "Sending message: " << msg << std::endl;
      server.send(connHandle, msg, websocketpp::frame::opcode::text);
    };
    const auto closeConnection = [&server, connHandle, &connIdToRelay, connId]() {
      server.close(connHandle, websocketpp::close::status::normal, "Done");
      connIdToRelay.erase(connId);
    };
    auto& relay = connIdToRelay.emplace(
      connId, RelayToConnection(sendMessage, closeConnection, events)).first->second;
